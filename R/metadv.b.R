    ...
    private = list(
        .run = function() {
            ...
            if (ready == TRUE) {
                ...
                if (self$options$moderatorType == "NON") {
                    ...
                    data <- data.frame(yi = self$data[[self$options$effectSize]],
                    ...
                    resTOSTText <- capture.output(
                        TOSTER::TOSTmeta(ES = res$beta,
                                         se = res$se,
                                         low_eqbound_d = lowerTOST,
                                         high_eqbound_d = upperTOST,
                                         alpha = alphaTOST,
            ...
            outputTextTOST <- paste(resTOSTText[18],
                                    resTOSTText[20],
                                    resTOSTText[21], 
                                    sep = "\n")
            ...
            #Pub Bias
            failsafePB <- metafor::fsn(yi = res$yi,
                                       vi = res$vi,
                                       type = fsntype)
            ...
            fsnRICH$setRow(rowNo = 1,
                           values = list(label = "Fail-Safe N",
                                         failSafeNumber = failsafePB$fsnum[1],
                                         p = failsafePB$pval[1]))

            fsnRICH$setRow(rowNo = 2,
                           values = list(label = "Kendalls Tau",
                                         failSafeNumber = ranktestPB$tau[1],
                                         p = ranktestPB$pval[1]))

            fsnRICH$setRow(rowNo = 3,
                           values = list(label = "Egger's Regression",
                                         failSafeNumber = regtestPB[["zval"]],
                                         p = regtestPB[["pval"]]))
            
            fsnTitle <- paste("Publication Bias Assessment")
            fsnNote <- paste("Fail-safe N Calculation Using the ",
                             fsntype,
                             " Approach",
                             sep = "")
            fsnRICH$setTitle(title = fsnTitle)
            ...
            modelFitRICH$setRow(rowNo = 1,
                                values = list(label = "Maximum-Likelihood",
            ...
            modelFitRICH$setRow(rowNo = 2,
                                values = list(label = "Restricted Maximum-Likelihood",
            ...
            table$setRow(rowNo = 1,
                         values = list(Intercept = "Intercept",
            ...
            if (is.null(self$options$moderatorcor) == FALSE) {
                ...
                table$setRow(rowNo = 2,
                             values = list(Intercept = "Moderator",
            ...
            tableTauSqaured$setRow(rowNo = 1,
                                   values = list(tauSqComb = tauSqCombind,
                                                 tauSQRT = tauOnly,
                                                 ISqu = ISquStat,
                                                 HSqu = HSquStat,
                                                 RSqu = RSquStat,
      ...
      .tostplot = function(imageTOST, ...) {
          ...
          if (ready == TRUE) {
              ...
              tostPlotFUN <- plotTOSTfunction(#ES = plotDataTOST$ES,
                                              #se = plotDataTOST$se,
